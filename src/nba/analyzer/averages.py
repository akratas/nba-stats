...
insert_query: str = 'INSERT INTO NBA.running_player_averages AS avgs ({headers}) ' \
                    'VALUES (' \
                    '%(game_date)s' \
                    ', %(player)s' \
                    ', %(minutes_played)s' \
                    ...
                    ', %(period_length)s' \
                    ')' \
                    'ON CONFLICT (game_date, player, period_length)' \
                    'DO UPDATE SET' \
                    ...
                    'AND avgs.period_length = %(period_length)s'
...
class DynamicAveragesCalculator:
    ...
    @staticmethod
    def calculate_averages(stats: Dict[str, Dict[str, any]], rows: List[Dict[str, any]],
                           columns: List[str], period: AveragePeriods) -> List[Dict[str, any]]:
        multiplier: int = period.value
        averages: List[Dict[str, any]] = []
        for row in rows:
            player: str = row.get('player')
            stat: Dict[str, any] = stats.get(player)
            if not stat:
                stat = {'period_length': multiplier, 'player': player}
                stats[player] = stat
...
class StatsLoader:
    ...
    game_date_query: str = 'SELECT * FROM NBA.game_stats g WHERE g.game_date BETWEEN \'{from_date}\' AND \'{to_date}\''
    season_query: str = 'SELECT s.season_start, s.season_end FROM NBA.seasons s WHERE s.season = \'{season}\''
    headers: str = 'SELECT column_name FROM information_schema.columns WHERE table_schema = \'nba\' ' \
                   'AND table_name = \'running_player_averages\''
    numeric_cols_names: str = 'SELECT column_name FROM information_schema.columns WHERE table_schema = \'nba\' ' \
                              'AND table_name = \'running_player_averages\' AND data_type = \'double precision\';'
    averages_player: str = 'SELECT player, minutes_played, field_goals, three_points, free_throws, offensive_rebounds' \
                           ', defensive_rebounds, total_rebounds, assists, steels, blocks, turn_overs, points_scored' \
                           ', overall_efficiency, period_length FROM NBA.running_player_averages a ' \
                           'WHERE a.period_length={period} AND a.game_date=(SELECT MAX(game_date) ' \
                           'FROM NBA.running_player_averages WHERE period_length={second_period})'
    ...
    def load_by_season(self, season: str) -> List[Dict[str, any]]:
        ...
        result: List[Dict[str, any]] = self.depot.do_query_many_dict(StatsLoader.season_query.format(season=season))
        if len(result) == 0:
    ...
    def load_by_dates(self, from_date: date, to_date: date) -> List[Dict[str, any]]:
        ...
        return self.depot.do_query_many_dict(StatsLoader.game_date_query.format(from_date=from_date, to_date=to_date))

    def get_averages_headers(self) -> str:
        rows: List[Dict[str, any]] = self.depot.do_query_many_dict(StatsLoader.headers)
        size: int = len(rows) - 1
        pointer: int = 0
        headers: str = ''
        for row in rows:
    ...
    def get_averages_column_names(self) -> List[str]:
        rows: List[Dict[str, any]] = self.depot.do_query_many_dict(StatsLoader.numeric_cols_names)
        columns: List[str] = []
        for row in rows:
    ...
    def load_previous_player_stats(self, period: AveragePeriods) -> Dict[str, Dict[str, any]]:
        rows: List[Dict[str, any]] = \
            self.depot.do_query_many_dict(StatsLoader.averages_player.format(period=period.value,
...
class AnalyticsController:
    ...
    def analyze_and_store(self, rows: List[Dict[str, any]]) -> None:
        ...
        for period in AveragePeriods:
            ...
            self.depot.do_batch_query_single_dict(insert_query.format(headers=headers), False, result)
